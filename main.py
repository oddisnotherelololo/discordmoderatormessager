...
from logic import Pokemon, TungstenWizard, ASSassin
import random
...
async def go(ctx):
    ...
    if author not in Pokemon.pokemons:  # Periksa apakah pengguna ini memiliki Pokémon 
        ...
        if chance == 1:
            ...
            pokemon = TungstenWizard(author)  # Membuat Pokémon Wizard 
        elif chance == 3:
            pokemon = ASSassin(author)  # Membuat Pokémon Fighter 
...
@bot.command()
async def start(ctx):
    await ctx.send("Type !go to do something, i don't know.")
